...
class _VideoCallState extends State<VideoCall> {
  ...
  onetooneview(double h, double w, bool iscallended, bool userenlarged) {
    ...
    if (iscallended == true) {
      return Container(
        color:
            DESIGN_TYPE == Themetype.whatsapp ? primaryColors : fiberchatWhite,
        height: h,
        width: w,
        child: Center(
            child: Icon(
  ...
  void _onCallEnd(BuildContext context) async {
    ...
    if (isalreadyendedcall == false) {
      ...
      FirebaseFirestore.instance
          ...
          .get()
          .then((value) {
        if(value['ENDED'] != null || value['STARTED'] != null){
          final second = value['ENDED']
              .toDate()
              .difference(
              value['STARTED']
                  .toDate())
          ...
          // if(second > 30){
          //   double creditForAudio = second / 30;
          //   creditForAudio = double.parse(creditForAudio.toStringAsFixed(2));
          //
          //   // FirebaseFirestore.instance
          //   //     .collection(DbPaths.collectionusers)
          //   //     .doc(widget.call.callerId)
          //   //     .set({
          //   //   Dbkeys.credits: FieldValue.increment(-creditForAudio),
          //   // }, SetOptions(merge: true));
          //   // print('called inside' + second.toString());
          //   // print('time '+widget.call.timeepoch.toString());
          //    // FirebaseFirestore.instance
          //    //    .collection(DbPaths.collectionusers)
          //    //    .doc(widget.call.receiverId)
          //    //    .collection(DbPaths.collectioncallhistory)
          //    //    .doc(widget.call.timeepoch.toString())
          //    //    .set({'CREDITS': creditForAudio}, SetOptions(merge: true));
          //    print('after ');
          //   // FirebaseFirestore.instance
          //   //     .collection(DbPaths.collectionusers)
          //   //     .doc(widget.call.callerId)
          //   //     .collection(DbPaths.collectioncallhistory)
          //   //     .doc(widget.call.timeepoch.toString())
          //   //     .set({'CREDITUSAGE':creditForAudio}, SetOptions(merge: true));
          // }
          //
        }
      });
  ...
  startTimerNow() {
    ...
    timerSubscription = timerStream!.listen((int newTick) async {
      ...
      final resultForStatus = await FirebaseFirestore.instance
          .collection(DbPaths.collectionusers)
          .doc(widget.call.callerId)
          .collection(DbPaths.collectioncallhistory)
          .doc(widget.call.timeepoch.toString())
      ...
      if (int.parse(secondsStr) % 30 == 0 &&
        ...
        final result = await FirebaseFirestore.instance
            ...
            .doc(widget.currentuseruid)
            .get();
        print(result['credits'].toString());
        if (result['credits'] <= 0) {
          print('in if end call');
          Fiberchat.toast('Not enough credit');
          streamController!.done;
          streamController!.close();
          timerSubscription!.cancel();
          ...
          print('in else');
          if (resultForStatus['STATUS'] != 'ended') {
            await FirebaseFirestore.instance
                .collection(DbPaths.collectionusers)
                .doc(widget.call.callerId)
                .set({
              Dbkeys.credits: FieldValue.increment(-1),
            }, SetOptions(merge: true));
            ...
            await FirebaseFirestore.instance
                ...
                .collection(DbPaths.collectioncallhistory)
                .doc(widget.call.timeepoch.toString())
                .set({'CREDITUSAGE': FieldValue.increment(1)},
                SetOptions(merge: true));
          }
        }
      }
      ...
      //           .collection(DbPaths.collectioncallhistory)
      //           .doc(widget.call.timeepoch.toString())
      //           .set({'CREDITUSAGE':FieldValue.increment(1)}, SetOptions(merge: true));
      //   }
      // }
      flutterLocalNotificationsPlugin.cancelAll();
    });

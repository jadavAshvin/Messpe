...
class Fiberchat {
  ...
  static void invite(BuildContext context)async {

    final sp = await SharedPreferences.getInstance();
    final phoneNo = sp.getString(Dbkeys.phone)!;
    final result =await  FirebaseFirestore.instance
        .collection(DbPaths.collectionusers)
        .doc(phoneNo).get() ;
    print(result['referralCode']);
    final observer = Provider.of<Observer>(context, listen: false);
    String multilingualtext = Platform.isIOS
        ? '${getTranslated(context, 'letschat')} $Appname, ${getTranslated(context, 'joinme')} - ${observer.iosapplink} - Referral code ${result['referralCode']}'
        : '${getTranslated(context, 'letschat')} $Appname, ${getTranslated(context, 'joinme')} -  ${observer.androidapplink} - Referral code ${result['referralCode']}';
  ...
  static Widget ClipRRectavatar(Map<String, dynamic>? user,
      ...
      return Container(
        ...
        decoration: BoxDecoration(
            ...
            image: DecorationImage(
                ...
                image:Image.file(File(user[Dbkeys.aliasAvatar])).image
            )
        ),
      );
    }

...
import 'package:fiberchat/Screens/first_time_screen/firstTimeScreen.dart';
...
class _FiberchatWrapperState extends State<FiberchatWrapper> {
  ...
  bool isFirstTimeChecked = false;
  Widget home = Scaffold(
    body: CircularProgressIndicator(),
  );
  Future<void> isFirsTimeScreen(snapshot, BuildContext context) async {
    print("called isFirsTimeScreen");
    final prefs = await SharedPreferences.getInstance();
    bool containsKeyForFirstTime = prefs.containsKey('isFirstTime');
    if (!prefs.containsKey('isFirstTime')) {
      await prefs.setBool('isFirstTime', true);

      home = FirstTimeScreen(
        currentUserNo: snapshot.data!.getString(Dbkeys.phone),
        isSecuritySetupDone:
            snapshot.data!.getString(Dbkeys.isSecuritySetupDone) == null
                ? false
                : ((snapshot.data!.getString(Dbkeys.phone) == null)
                    ? false
                    : (snapshot.data!.getString(Dbkeys.isSecuritySetupDone) ==
    ...
    if (this._locale == null) {
      ...
      return FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.connectionState == ConnectionState.done) {
              return FutureBuilder(
                  ...
                  builder:
                            ...
                            child: MaterialApp(
                              builder: (BuildContext? context, Widget? widget) {
                                if (!isFirstTimeChecked) {
                                  isFirsTimeScreen(snapshot, context!);
                                }
                                ErrorWidget.builder =
                    ...
                    print("Splash Screen called first time");
                    return MultiProvider(
                      providers: [
                        ChangeNotifierProvider(create: (_) => UserProvider()),
                      ],
            ...
            print("Splash Screen called second time");
            return MaterialApp(
              debugShowCheckedModeBanner: false,
              home: Splashscreen(),
            );
